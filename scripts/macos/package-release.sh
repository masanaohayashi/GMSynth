#!/usr/bin/env bash
# Build, notarize, staple, and publish the GMSynth macOS release.
#
...
JUCER_FILE="${REPO_ROOT}/GMSynth.jucer"
XCODE_PROJECT="${REPO_ROOT}/Builds/MacOSX/GMSynth.xcodeproj"
...
Builds a signed Universal macOS app, creates a DMG containing GMSynth.app and
...
The following environment variables can also be set in scripts/macos/config.env:
  ...
  BUILD_ARCHS, RELEASE_BRANCH, GH_REPO
EOF
}
...
check_repository() {
  ...
  [[ "$(git -C "$REPO_ROOT" rev-parse HEAD)" == "$remote_head" ]] \
    || die "HEAD is not equal to origin/$RELEASE_BRANCH; push the intended commit first"
  ...
  [[ -d "${REPO_ROOT}/external/JUCE/modules" ]] \
    || die "JUCE submodule is missing: external/JUCE/modules"
  [[ -d "${REPO_ROOT}/external/fluidsynth" ]] \
    || die "FluidSynth submodule is missing: external/fluidsynth"
}
...
check_release_target() {
  ...
  if git -C "$REPO_ROOT" ls-remote --exit-code --refs origin "refs/tags/$RELEASE_TAG" \
    >/dev/null 2>&1; then
...
archive_app() {
  ...
  log "Archiving GMSynth - Standalone Plugin (${BUILD_CONFIGURATION}, ${BUILD_ARCHS})"
  if ! xcodebuild archive \
    -project "$XCODE_PROJECT" \
    -scheme "GMSynth - Standalone Plugin" \
  ...
  [[ -d "$APP_PATH/Contents/PlugIns/GMSynth.appex" ]] \
  ...
  signature_info="$(codesign --display --verbose=4 "$APP_PATH" 2>&1)" || die "codesign display failed"
...
create_dmg() {
  ...
  COPYFILE_DISABLE=1 ditto --norsrc --noextattr --noqtn \
    "$APP_PATH" "$DMG_STAGE/GMSynth.app"
...
verify_dmg_contents() {
  ...
  [[ -d "$MOUNT_POINT/GMSynth.app" ]] || die "DMG is missing GMSynth.app"
  ...
  codesign --verify --deep --strict --verbose=2 "$MOUNT_POINT/GMSynth.app"
...
main() {
  ...
  if [[ "$RELEASE_TAG" != "v${version}" \
    ...
    die "release tag must match the GMSynth.jucer version (${version}): $RELEASE_TAG"
  fi
  RELEASE_TITLE="GMSynth ${RELEASE_TAG}"

  DIST_DIR="${REPO_ROOT}/dist"
  WORK_DIR="${DIST_DIR}/work/${RELEASE_TAG}"
  ARCHIVE_PATH="${WORK_DIR}/GMSynth.xcarchive"
  APP_PATH="${ARCHIVE_PATH}/Products/Applications/GMSynth.app"
  DMG_STAGE="${WORK_DIR}/dmg-root"
  DMG_VOLUME_NAME="GMSynth ${version}"
  DMG_PATH="${DIST_DIR}/GMSynth-${version}-macOS.dmg"
